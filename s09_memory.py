...
Flow in agent_loop:
    1. Load MEMORY.md index into SYSTEM prompt
    2. Select relevant memories → inject content
    3. Run compression pipeline from s08
    4. After each turn → extract new memories
    5. Periodically consolidate (Dream)

Changes from s08:
  + MEMORY_DIR, MEMORY_INDEX configs
  + _parse_frontmatter() — parse YAML frontmatter
  + write_memory_file() — write memory with frontmatter
  + _rebuild_index() — rebuild MEMORY.md from files
  + read_memory_index() — read index for SYSTEM
  + read_memory_file() — read single memory file
  + list_memory_files() — list all memories
  + select_relevant_memories() — LLM-based selection
  + load_memories() — load relevant content
  + extract_memories() — extract new memories
  + consolidate_memories() — merge duplicates
  + build_system() — includes memory index
  + agent_loop() — inject + extract + consolidate

Run: python s09_memory.py
