...
Execution order: budget → snip → micro → auto.

Changes from s07:
  + snip_compact() — trim middle messages
  + micro_compact() — old result placeholders
  + tool_result_budget() — persist large results
  + compact_history() — LLM summary
  + reactive_compact() — emergency compact
  + compact tool — user can trigger compaction
  + CONTEXT_LIMIT, KEEP_RECENT, PERSIST_THRESHOLD configs

Run: python s08_context_compact.py
...
def reactive_compact(messages):
    """紧急压缩：当 API 返回 prompt_too_long 错误时"""
...
def agent_loop(messages: list):
    ...
    while True:
        ...
        for block in response.content:
            ...
            # s08: compact tool triggers compact_history
